...
export default function SignUpPage() {
  ...
  const presetInterests = ["fotbal","hudba","cestování","programování","filmy","knihy","jazyk","hudební nástroj"];
          ...
          <div className="card-body space-y-5 text-center">
            ...
            {step === 1 && (
              <div className="space-y-2 text-left">
                ...
                <ul className="text-xs space-y-1 mt-1">
                  <li className={`${(!passTouched && !password) ? 'text-gray-300' : (passLenOk ? 'text-green-400' : 'text-red-400')}`}>• Minimálně 8 znaků</li>
                  <li className={`${(!passTouched && !password) ? 'text-gray-300' : (passUpperOk ? 'text-green-400' : 'text-red-400')}`}>• Alespoň jedno velké písmeno (A–Z)</li>
                  <li className={`${(!passTouched && !password) ? 'text-gray-300' : (passNumOk ? 'text-green-400' : 'text-red-400')}`}>• Alespoň jedno číslo (0–9)</li>
                </ul>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-4 text-left">
                ...
                {role==='TEACHER' && (
                  <input className="input" placeholder="Ověřovací kód učitele" value={teacherCode} onChange={e=>setTeacherCode(e.target.value)} />
                )}
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-2 text-sm text-left">
                ...
                <input className="input mt-2" placeholder="Zadejte znovu heslo" type="password" value={passwordConfirm} onChange={e=>setPasswordConfirm(e.target.value)} />
              </div>
            )}
            ...
            <div className="flex gap-2 justify-center">
              ...
              {!ok && (
                <button className="btn btn-primary" onClick={async()=>{
                  ...
                  setOk(false);
                  const res = await fetch("/api/auth/register", { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ name, email, password, birthDate, interests, phoneCode, phoneNumber, role, teacherCode }) });
